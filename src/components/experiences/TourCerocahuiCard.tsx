...
import { TourCerocahuiModal } from './TourCerocahuiModal'
import { ExperienceQuoteModal, buildExperienceQuoteData } from './ExperienceQuoteModal'
import Image from 'next/image'
import { useTranslation } from '@/contexts/LocaleContext'

export function TourCerocahuiCard({ tour }: { tour: any }) {
    ...
    return (
        <>
            ...
            >
                <div className="aspect-[4/3] bg-[#181410] overflow-hidden relative">
                    <div className="absolute inset-0 block">
                        <Image src="/images/experiences/tour-cerocahui-mirador-del-gallego-v2.jpg" alt="Tour Cerocahui y Mirador del Gallego" fill sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" className="w-full h-full object-cover transition-transform duration-[1.5s] ease-[0.33,1,0.68,1] group-hover:scale-110" />
                    </div>
                    ...
                    <div className="absolute bottom-4 left-4 flex items-center gap-2 text-white/90 text-xs font-medium">
                        <Clock size={13} />
                        {tour?.durationHours || 10} {t('common.horas')}
                    </div>
                </div>
                <div className="p-6">
                    ...
                    <h2 className="font-serif text-xl text-[#0a192f] mb-3 group-hover:text-[#2e4a3d] transition-colors">Tour Cerocahui y Mirador del Gallego</h2>
                    <p className="text-sm text-gray-500 leading-relaxed mb-5 line-clamp-2">{t('generic.salidaLugarAcordado')}</p>
                    ...
                    <div className="bg-gray-50 rounded-xl p-4 mb-5">
                        ...
                        <div className="space-y-1.5">
                            {(tour?.tierPrices?.length > 1
                                ...
                                : [
                                    { minPax: 1, maxPax: 2, pricePerPerson: 1700 },
                                    { minPax: 3, maxPax: 6, pricePerPerson: 1400 },
                                    { minPax: 7, maxPax: 11, pricePerPerson: 1300 },
                                ].map((fb, i) => (
                                    <div key={i} className="flex justify-between text-sm">
                                        <span className="text-gray-600">{fb.minPax}–{fb.maxPax} {t('common.pax')}</span>
                                        <span className="font-semibold text-[#0a192f]">${fb.pricePerPerson.toLocaleString()}</span>
                                    </div>
                                ))
                            )}
                        </div>
            ...
            <TourCerocahuiModal isOpen={isOpen} setIsOpen={setIsOpen} tour={tour} onRequestQuote={() => { setIsOpen(false); setQuoteOpen(true) }} />
